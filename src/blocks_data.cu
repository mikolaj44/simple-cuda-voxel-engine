...
__device__ void getPhongIllumination(Vector3 pos, Vector3 cameraPos, Vector3 normal, Material material, PointLight light, int& r, int& g, int&b){
	...
	Vector3 lighting = (add(Vector3(0, 0, 0), vmul(light.color, add(mul(diffuseVector, dot(normal, Ln)), mul(specularVector, pow(dot(h, dh), material.specularExponent))))));
	...
	lighting = vmul(lighting, Vector3(50,50,50));
