...
#define EPSILON 0.000000001f
	...
	__device__ inline unsigned int hash(unsigned int seed)
	{
	...
	__device__ inline int randomIntRange(int min, int max, int seed)
	{
		int base = hash(seed);
	...
	__device__ inline float randomFloat(unsigned int seed)
	{
	...
	// NOTE: no clamping
	__device__ inline float mapToSigned(float input)
	{
	...
	__device__ inline float randomGrid(int x, int y, int z, int seed = 0)
	{
	...
	__device__ inline unsigned int randomIntGrid(float x, float y, float z, float seed = 0.0f)
	{
		return hash((unsigned int)(x * 1723.0f + y * 93241.0f + z * 149812.0f + 3824 + seed));
	}

	// Random 3D vector as float3 from grid position
	__device__ inline float3 vectorNoise(int x, int y, int z)
	{
	...
	__device__ inline float3 scaleVector(float3 v, float factor)
	{
		return make_float3(v.x * factor, v.y * factor, v.z * factor);
	}

	// Scale 3D vector by nonuniform parameters
	__device__ inline float3 nonuniformScaleVector(float3 v, float xf, float yf, float zf)
	{
	...
	// Adds two 3D vectors
	__device__ inline float3 addVectors(float3 v, float3 w)
	{
		return make_float3(v.x + w.x, v.y + w.y, v.z + w.z);
	}

	// Dot product between two vectors
	__device__ inline float dotProduct(float3 u, float3 v)
	{
	...
	__device__ inline __constant__ float gradMap[16][3] = { { 1.0f, 1.0f, 0.0f },{ -1.0f, 1.0f, 0.0f },{ 1.0f, -1.0f, 0.0f },{ -1.0f, -1.0f, 0.0f },
	...
	__device__ inline  float lerp(float a, float b, float ratio)
	{
		return a * (1.0f - ratio) + b * ratio;
	}

	// 1D cubic interpolation with four points
	__device__ inline float cubic(float p0, float p1, float p2, float p3, float x)
	{
	...
	__device__ inline float dot(float g[3], float x, float y, float z) {
		return g[0] * x + g[1] * y + g[2] * z;
	}

	// Random value for simplex noise [0, 255]
	__device__ inline unsigned char calcPerm(int p)
	{
		return (unsigned char)(hash(p));
	}

	// Random value for simplex noise [0, 11]
	__device__ inline unsigned char calcPerm12(int p)
	{
	...
	__device__ inline float simplexNoise(float3 pos, float scale, int seed)
	{
	...
	// Checker pattern
	__device__ inline float checker(float3 pos, float scale, int seed)
	{
	...
	// Random spots
	__device__ inline float spots(float3 pos, float scale, int seed, float size, int minNum, int maxNum, float jitter, profileShape shape)
	{
		if (size < EPSILON)
			return 0.0f;
	...
	__device__ inline float worleyNoise(float3 pos, float scale, int seed, float size, int minNum, int maxNum, float jitter)
	{
		if (size < EPSILON)
			return 0.0f;
	...
	__device__ inline float tricubic(int x, int y, int z, float u, float v, float w)
	{
	...
	__device__ inline float discreteNoise(float3 pos, float scale, int seed)
	{
		int ix = (int)(pos.x * scale);
		int iy = (int)(pos.y * scale);
		int iz = (int)(pos.z * scale);
	...
	// Linear value noise
	__device__ inline float linearValue(float3 pos, float scale, int seed)
	{
	...
	__device__ inline float fadedValue(float3 pos, float scale, int seed)
	{
		float fseed = (float)seed;
	...
	__device__ inline float cubicValue(float3 pos, float scale, int seed)
	{
		pos.x = pos.x * scale;
		pos.y = pos.y * scale;
		pos.z = pos.z * scale;
	...
	__device__ inline float perlinNoise(float3 pos, float scale, int seed)
	{
		float fseed = (float)seed;
...
// Derived noise functions
	...
	__device__ inline float repeaterPerlin(float3 pos, float scale, int seed, int n, float lacunarity, float decay)
	{
		float acc = 0.0f;
		float amp = 1.0f;
    ...
    // Fast function for fBm using perlin noise
    __device__ inline float repeaterPerlinBounded(float3 pos, float scale, int seed, int n, float lacunarity, float decay, float threshold)
    {
        float acc = 1.0f;
        float amp = 1.0f;
	...
	__device__ inline float repeaterPerlinAbs(float3 pos, float scale, int seed, int n, float lacunarity, float decay)
	{
		float acc = 0.0f;
		float amp = 1.0f;
	...
	__device__ inline float repeaterSimplex(float3 pos, float scale, int seed, int n, float lacunarity, float decay)
	{
		float acc = 0.0f;
		float amp = 1.0f;
	...
	__device__ inline float repeaterSimplexAbs(float3 pos, float scale, int seed, int n, float lacunarity, float decay)
	{
		float acc = 0.0f;
		float amp = 1.0f;
    ...
    __device__ inline float repeaterSimplexBounded(float3 pos, float scale, int seed, int n, float lacunarity, float decay, float threshold)
    {
        float acc = 1.0f;
        float amp = 1.0f;
	...
	__device__ inline float repeater(float3 pos, float scale, int seed, int n, float lacunarity, float decay, basisFunction basis)
	{
		float acc = 0.0f;
		float amp = 1.0f;
	...
	__device__ inline float fractalSimplex(float3 pos, float scale, int seed, float du, int n, float lacunarity, float decay)
	{
		float acc = 0.0f;
		float amp = 1.0f;
	...
	__device__ inline float turbulence(float3 pos, float scaleIn, float scaleOut, int seed, float strength, basisFunction inFunc, basisFunction outFunc)
	{
		switch (inFunc)
		{
		case(BASIS_CHECKER):
	...
	__device__ inline float repeaterTurbulence(float3 pos, float scaleIn, float scaleOut, int seed, float strength, int n, basisFunction basisIn, basisFunction basisOut)
	{
